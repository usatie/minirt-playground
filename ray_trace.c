...
t_fcolor	*ray_trace_perfect_reflection(t_intersection_test_result *res, t_ray *ray, t_scene *scene, int recursion_level)
{
	...
	if (R_re)
		return (fcolor_mul(res->shape->material->catadioptric_factor, R_re));
	return (NULL);
}

t_fcolor	*ray_trace_refraction(t_intersection_test_result *res, t_ray *ray, t_scene *scene, int recursion_level)
{
...
t_fcolor	*ray_trace_recursive(t_scene *scene, t_ray *ray, int recursion_level)
{
	...
	if (res)
	{
		...
		{
			t_fcolor	*R_mirror = ray_trace_perfect_reflection(res, ray, scene, recursion_level);
			if (R_mirror)
				R = fcolor_add(R, R_mirror);
		}
		if (res->shape->material->use_refraction)
		{
			t_fcolor	*R_refrac = ray_trace_refraction(res, ray, scene, recursion_level);
			if (R_refrac)
				R = fcolor_add(R, R_refrac);
		}
		return (R);
	}
	else
